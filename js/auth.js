...
(function () {
  ...
  // 解除「單一裝置」的登記（登出時釋放這台名額）。回傳 true/false。需後端 unregister_device RPC。
  function unregisterDevice(email, device) {
    email = (email || "").trim().toLowerCase();
    if (!sb || !email || !device) return Promise.resolve(false);
    return sb.rpc("unregister_device", { p_email: email, p_device: device })
